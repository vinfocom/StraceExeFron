      ...
      <CardContent className="p-4 sm:p-5 lg:p-6">
        <div className="flex flex-wrap items-start gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-[1_1_10rem] [overflow-wrap:anywhere]">
            <p className="text-2xl font-bold text-gray-900 leading-tight sm:text-3xl">
              {Number(value ?? 0).toLocaleString()}
            </p>
            <p className="text-sm text-gray-800 font-semibold mt-1">{title}</p>
            {description && (
            ...
            {trend && (
              <div className="flex items-center gap-1 mt-2">
                ...
                <span className="min-w-0 text-xs text-green-600 font-semibold">{trend}% increase</span>
              </div>
            )}
          </div>
        </div>
