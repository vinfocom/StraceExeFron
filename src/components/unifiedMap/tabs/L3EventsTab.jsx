...
function DiagnosticApiOverview({ summary }) {
  ...
  const hasOverview = dashboardValues.length || summary.totalRows != null || summary.l3Rows != null || summary.eventRows != null;
      ...
      {dashboardValues.length > 0 && (
        <div className="mt-3 overflow-x-auto rounded border border-slate-700">
          <table className="w-full min-w-[520px] border-collapse text-xs">
            ...
            <tbody>{dashboardValues.map((item, index) => <tr key={`${item.parameter}-${index}`} className="border-b border-slate-800/90 last:border-b-0"><td className="border-r border-slate-800 px-2 py-1.5 text-slate-200">{item.parameter || "—"}</td><td className="px-2 py-1.5 text-slate-300">{item.result || "—"}</td></tr>)}</tbody>
          </table>
        </div>
      )}
    </section>
